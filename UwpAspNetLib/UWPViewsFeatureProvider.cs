...
namespace UwpAspNetLib
{
    ...
    public class UWPViewsFeatureProvider : IApplicationFeatureProvider<ViewsFeature>
    {
        public static readonly string PrecompiledViewsAssemblySuffix = ".Views";
        ...
        private static Assembly GetFeatureAssembly(AssemblyPart assemblyPart)
        {
            ...
            if (File.Exists(precompiledAssemblyFilePath))
            {
                try
                {
                    return Assembly.Load(assemblyPart.Assembly.GetName().Name + PrecompiledViewsAssemblySuffix);
                }
